...
export async function logout(req,res) {
    try {
        res.clearCookie("jwt-netstream");
        res.status(200).json({success: true, message:"Logged out successfully"});
    } catch(error) {
        console.log('Error in logout controller', error.message)
        res.status(500).json({success: false, message:"Internal server error while logout"});
    }
}
...
export async function authCheck(req,res) {
    try {
        ...
        res.status(500).json({success: false, message:"Internal server error while authCheck"});
    }
}